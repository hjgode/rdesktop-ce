...
mi_create_window(void)
{
  ...
  {
#ifdef MYWINCE
	  DEBUGMSG(1, ("Failed to register window class"));
#else
	OutputDebugStr("Failed to register window class");
#endif
    return 0; /* Failed to register window class */
  }
  rc.left = 0; rc.right = 0; rc.top = 0; rc.bottom = 0;
  style = WS_POPUP;
  //style = WS_POPUPWINDOW; // WS_POPUP | WS_BORDER;// | WS_CAPTION | WS_SYSMENU;
  exStyle = WS_EX_TOPMOST; // WS_EX_NODRAG | WS_EX_CAPTIONOKBTN | WS_EX_WINDOWEDGE;
...
#else /* MYWINCE */
  str_to_uni(filename, "d:\\\winrdesktop.ini");
#endif /* MYWINCE */
